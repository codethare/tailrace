...
#[test]
fn single_output_keeps_partial_window_without_resizing() {
    ...
    let edge = s.state.wm.outputs[0].workspace_list[0].window_list[1]
        .geom
        .current;
    assert!(edge.x + edge.width > output.x + output.width);
    assert_eq!(count_requests(&sv, &right_window, 4), 0);
    assert_eq!(count_requests(&sv, &right_window, 3), 0);
    assert_eq!(count_requests(&sv, &left_window, 3), 0);
    assert_eq!(
        s.state.wm.outputs[0].workspace_list[0].window_list[1]
            .geom
            .sent_visible,
        Some(true),
        "a partial window must stay visible on a single output"
    );
}

#[test]
fn multi_output_hides_cross_output_overhang_without_resizing() {
    let (mut s, mut sv) = build();
    s.add_seat(&mut sv);
    s.manage(&mut sv);
    s.add_output(&mut sv, "A", (0, 0), (1360, 768));
    ...
    assert_eq!(count_requests(&sv, &right_window, 4), 1);
    assert_eq!(count_requests(&sv, &right_window, 3), 0);
    assert_eq!(count_requests(&sv, &left_window, 3), 0);
    ...
    assert_eq!(count_requests(&sv, &right_window, 5), 1);
    assert_eq!(count_requests(&sv, &right_window, 3), 0);
    assert_eq!(
        s.state.wm.outputs[0].workspace_list[0].window_list[1]
            .geom
            .sent_visible,
        Some(true),
        "focusing the edge window must show it again"
    );
}
